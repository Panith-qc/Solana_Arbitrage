...
export class FinalRobustTradeExecutor {
    constructor() {
        ...
        Object.defineProperty(this, "solPriceCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "PRICE_CACHE_TTL", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 30000
    ...
    async getSOLPriceUSD() {
        ...
        try {
            ...
            const quote = await multiAPIService.getQuote(SOL_MINT, USDC_MINT, 1 * LAMPORTS_PER_SOL, 100 // 1% slippage for quote
            );
    ...
    async calculateTotalFees(inputMint, outputMint, amount, useJito = false) {
        try {
            ...
            const conservativeFeeLamports = 10000; // ~0.00001 SOL
            const solPriceUSD = await this.getSOLPriceUSD();
            return {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    validateTradePair(inputMint, outputMint) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async verifyTokenAccount(wallet, tokenMint, expectedMinimum = 0n) {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            try {
                ...
                const tokenAccounts = await this.connection.getTokenAccountsByOwner(wallet.publicKey, { mint: new PublicKey(tokenMint) });
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async qualityGate(tokenMint, amountSOL) {
        ...
        try {
            ...
            try {
                const forwardQuote = await multiAPIService.getQuote(SOL_MINT, tokenMint, amountSOL * LAMPORTS_PER_SOL, 100);
            ...
            try {
                ...
                const reverseQuote = await multiAPIService.getQuote(tokenMint, SOL_MINT, estimatedTokenOutput, 100);
    ...
    async executeTrade(params) {
        ...
        try {
            ...
            const fees = await this.calculateTotalFees(inputMintStr, outputMintStr, params.amount, params.useJito);
            console.log(`💸 Total Fees: ${fees.totalFeeSOL.toFixed(8)} SOL ($${fees.totalFeeUSD.toFixed(4)})`);
            // ✅ STEP 3: Get quote
            console.log('\n📊 Step 2: Getting quote...');
            const quote = await multiAPIService.getQuote(inputMintStr, outputMintStr, params.amount, params.slippageBps);
            if (!quote) {
            ...
            let signedTx;
            try {
                signedTx = await jupiterUltra.signUltraTransaction(orderResponse.transaction, params.wallet);
            }
            catch (error) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async executeArbitrageCycle(tokenMint, amountSOL, slippageBps, wallet, useJito = false) {
        console.log('\n\n');
        console.log('█████████████████████████████████████████████████████████');
