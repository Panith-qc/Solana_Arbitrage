...
import { RiskLevel, getRiskProfile, RiskProfile } from '../config/riskProfiles';
...
export class AutoConfigService {
   ...
   */
  private calculatePositionSizing(balance: number, profile: RiskProfile) {
    ...
    const maxDailyLossSol = Math.max(
      (balance * 5) / 100, // 5% of balance
      0.1 // Minimum 0.1 SOL
    );
   ...
   */
  private getEnabledStrategies(profile: RiskProfile): string[] {
    const strategies: string[] = [];
    const strats = profile.enabledStrategies;
    
    if (strats.backrun) strategies.push('Backrun');
    if (strats.cyclicArbitrage) strategies.push('Cyclic Arbitrage');
    if (strats.jitLiquidity) strategies.push('JIT Liquidity');
    if (strats.longTailArbitrage) strategies.push('Long-Tail Arbitrage');
    if (strats.microArbitrage) strategies.push('Micro Arbitrage');
    if (strats.crossDexArbitrage) strategies.push('Cross-DEX Arbitrage');
    if (strats.sandwich) strategies.push('Sandwich');
    if (strats.liquidation) strategies.push('Liquidation');
   ...
   */
  private validateConfiguration(balance: number, settings: any): string[] {
   ...
   */
  private logConfiguration(config: AutoConfig): void {
    console.log('\n╔════════════════════════════════════════════════════════╗');
   ...
   */
  async quickSetup(walletAddress: string, riskLevel: RiskLevel): Promise<{
    config: AutoConfig;
