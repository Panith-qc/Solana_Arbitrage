...
export class RealJupiterTrading {
    ...
    async getSwapTransaction(quote) {
        try {
            const wallet = realSolanaWallet.getWallet();
            if (!wallet?.publicKey) {
        ...
        try {
            ...
            for (const pair of tradingPairs) {
                try {
                    ...
                    const forwardQuote = await this.getQuote(pair.inputMint, pair.outputMint, pair.baseAmount);
                    if (!forwardQuote)
                        continue;
                    // Get quote for reverse trade
                    const reverseQuote = await this.getQuote(pair.outputMint, pair.inputMint, parseInt(forwardQuote.outAmount));
                    if (!reverseQuote)
                        continue;
