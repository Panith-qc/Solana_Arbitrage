...
export interface MultiHopOpportunity {
  id: string;
  path: string[]; // ['SOL', 'USDC', 'BONK', 'SOL']
  mints: string[]; // Mint addresses for each hop
  profitSOL: number;
  profitUSD: number;
...
export class MultiHopArbitrageService {
  ...
  private readonly MIN_PROFIT_USD = 0.05; // $0.05 minimum profit for multi-hop
  private readonly MAX_HOPS = 3; // Maximum 3-hop cycles (SOL → A → B → SOL)

  /**
   * Scan for 3-hop arbitrage opportunities
   * More complex but more profitable than 2-hop
   */
  async scan3HopOpportunities(
    tokens: TokenInfo[],
    ...
    for (let i = 0; i < topTokens.length; i++) {
      for (let j = 0; j < topTokens.length; j++) {
        ...
        try {
          ...
          // HOP 1: SOL → TokenA
          const hop1 = await multiAPIService.getQuote(
            this.SOL_MINT,
            tokenA.mint,
            scanAmountLamports,
            50
          ...
          if (!hop1?.outAmount) continue;

          // HOP 2: TokenA → TokenB
          const hop2 = await multiAPIService.getQuote(
            tokenA.mint,
            tokenB.mint,
            Number(hop1.outAmount),
            50
          );

          if (!hop2?.outAmount) continue;

          // HOP 3: TokenB → SOL
          const hop3 = await multiAPIService.getQuote(
            tokenB.mint,
            this.SOL_MINT,
            Number(hop2.outAmount),
            50
          );

          if (!hop3?.outAmount) continue;
          ...
          if (netProfitUSD >= this.MIN_PROFIT_USD) {
            ...
            const opportunity: MultiHopOpportunity = {
  ...
  async scan2HopOpportunities(
    ...
    opportunities.push(...results.filter(r => r !== null) as MultiHopOpportunity[]);
  ...
  async executeMultiHop(
    ...
    wallet: any
  ): Promise<{ success: boolean; actualProfitUSD: number }> {
