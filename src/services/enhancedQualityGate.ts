...
export interface QualityCheckResult {
  ...
  reason?: string;
  checks: {
    priceImpact: { passed: boolean; value: number; threshold: number };
    liquidity: { passed: boolean; value: number; threshold: number };
    volatility: { passed: boolean; value: number; threshold: number };
    profitMargin: { passed: boolean; value: number; threshold: number };
  };
}

export class EnhancedQualityGate {
  // Thresholds for quality checks
  private readonly MAX_PRICE_IMPACT = 0.5; // 0.5% max price impact
  private readonly MIN_LIQUIDITY_MULTIPLIER = 10; // Liquidity must be 10x trade size
  private readonly MAX_VOLATILITY = 5.0; // 5% max volatility in last 5 minutes
  private readonly MIN_PROFIT_MARGIN = 0.02; // $0.02 minimum profit after all fees

  /**
   * Comprehensive quality check before executing trade
   * Returns whether trade should proceed and detailed reasoning
   */
  async checkTradeQuality(
    tokenMint: string,
    ...
    try {
      ...
      if (priceImpact < 0.1) confidence += 0.05; // Very low price impact
      if (liquidityMultiplier > 20) confidence += 0.05; // Excellent liquidity
      if (volatility < 1.0) confidence += 0.05; // Very stable
